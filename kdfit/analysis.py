...
class Analysis:
    ...
    def minimize(self, verbose=False, show_steps=False, solver='standard', method=None, jac=None, **kwargs):
        """
        ...
        if solver == 'standard':
            ...
            if np.all(np.abs(bounds) == np.inf):
                minimum = opt.minimize(
                    partial(self, show_steps=show_steps, verbose=verbose),
                    x0=initial,
                    method=method,
                    jac=jac,
                    options={**kwargs},
            ...
            minimum = opt.dual_annealing(
                partial(self, show_steps=show_steps, verbose=verbose), x0=initial, bounds=bounds, **kwargs
            )
        else:
    ...
    def posterior_probability(self, minimum, post_param, log=True, interval=None, ndx=20, verbose=False, **kwargs):
            ...
            def get_limit(positive=False):
                ...
                while dnll < 5:
                    if positive:
                        ...
                        if post_param.value > post_param.constraints[1]:
                            return post_param.constraints[1]
                    else:
                        post_param.value = minimum.params[post_param] - dx * i
                        if post_param.value < post_param.constraints[0]:
                            return post_param.constraints[0]
                    test_min = self.minimize(**kwargs)
        ...
        try:
            ...
            for val_i, val in enumerate(vals):
                ...
                for mps in val_min.params:
                    mps.value = val_min.params[mps]
